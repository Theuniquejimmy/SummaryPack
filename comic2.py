...
async def generate_neural_audio(text, voice, filename="temp.mp3"):
...
def create_audio(text, voice_choice):
    ...
    try:
        ...
        loop.run_until_complete(generate_neural_audio(clean_text, voice_choice))
        with open("temp.mp3", "rb") as f: data = f.read()
        os.remove("temp.mp3")
        return data
    except: return None
