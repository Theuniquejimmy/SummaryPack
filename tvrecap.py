...
# --- TIER 1: BUFFY-PROOF FANDOM SCRAPER ---
def get_raw_lore(wiki_slug, ep_title):
    ...
    try:
        # STEP 1: Direct Hit Strategy (Crucial for Buffy)
        # We try to parse the page directly using the episode title as the key
        parse_params = {
            "action": "parse",
            "page": ep_title,
            "prop": "text",
        ...
        # If direct hit fails, fallback to search but exclude transcripts
        if "error" in parse_res:
            ...
            parse_params["page"] = target_title
        ...
        # Clean up UI elements, infoboxes, and references
        for junk in soup.find_all(['table', 'aside', 'div', 'span', 'sup'], class_=['portable-infobox', 'navbox', 'mw-editsection', 'reference']):
            junk.decompose()
        
        content = []
        in_story = False
        # Sections where the plot usually hides
        story_keys = ['plot', 'synopsis', 'summary']
        # Sections to stop at so we don't get 50 pages of "Music" and "Trivia"
        ...
        for tag in soup.find_all(['h2', 'h3', 'p']):
            ...
            # Start collecting when we hit a story header
            if any(key in text for key in story_keys):
                in_story = True
                continue
            
            # Stop collecting if we hit a "junk" section header
...
def build_season_epub(show_id, show_name, season_num, wiki_slug):
    ...
    for i, ep in enumerate(season_episodes):
        ...
        content_body = final_text if final_text else "<p>Lore not found for this episode.</p>"
        c.content = f"<h2>{ep.get('name', 'Episode')}</h2>{content_body}"
        book.add_item(c)
        chapters.append(c)
...
if query:
    try:
        ...
        if resp:
            ...
            # Alias Lookup (Handles Buffy/Invincible perfectly)
            ...
            if st.session_state.lore_text:
                ...
                if st.button("🔊 Play Narration"):
                    # Use BeautifulSoup to clean the HTML for the text-to-speech engine
                    txt_to_read = BeautifulSoup(st.session_state.lore_text, "html.parser").get_text(separator=' ')
                    create_audio(txt_to_read, voice)
